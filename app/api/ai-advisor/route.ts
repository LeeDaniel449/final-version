  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("API key")) {
      return NextResponse.json(
        {
          reply:
            "I'm having trouble connecting to the AI service. Please make sure the OpenAI API key is configured correctly.",
        },
        { status: 500 },
      )
    }
...
function buildUserContext(userData?: UserData): string {
  ...
  // Budget Categories and Entries
  if (userData.budgetCategories && userData.budgetCategories.length > 0) {
    context += `BUDGET CATEGORIES:\n`
    userData.budgetCategories.forEach((category: any) => {
      context += `- ${category.name}: $${category.budgeted.toLocaleString()} budgeted, $${category.spent.toLocaleString()} spent\n`
    })
    context += `\n`
  }
